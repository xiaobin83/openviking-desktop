...
- 如需重新运行向导，删除首次运行标记文件即可（保留已安装的 Python 环境和工作目录数据）：
  - **macOS / Linux**：删除 `~/.openviking/.onboarded`（或 `~/Library/Application Support/com.openviking.desktop/.onboarded`）
  - **Windows**：删除 `%APPDATA%\com.openviking.desktop\.onboarded`（或 `%USERPROFILE%\.openviking\.onboarded`）
